...
//     // Cache to avoid unnecessary Firestore writes for RecordEarnings
...
//     // Statistics now sourced ONLY from RecordEarnings collection
...
//     // Overtime global settings (shared across users)
//     const [overtimeSettings, setOvertimeSettings] = useState({ thresholdHours: 40, overtimePercent: 50 })
//     // Modal state removed; overtime settings are managed globally on the main screen

//     // Listener for RecordEarnings (single source of truth for aggregates)
//     useEffect(() => {
//         if (!userId) return;
...
//     // Load global overtime settings from SystemSettings/OvertimeRules
//     useEffect(() => {
...
//                 } else {
//                     // Initialize default overtime settings if not present
//                     await setDoc(ref, {
//                         thresholdHours: 40,
//                         overtimePercent: 50,
...
//     // No local save; only reading global settings

//     // Recompute weekly and month-to-date statistics from RecordEarnings only
//     useEffect(() => {
//         if (!recordEarnings || recordEarnings.length === 0) {
//             setWeeklyStats({ scheduledHours: 0, workedHours: 0, efficiency: 0, weeklyEarnings: 0, regularHours: 0, overtimeHours: 0, regularEarnings: 0, overtimeEarnings: 0, thresholdCrossed: false });
//             return;
//         }
...
//         // Month-to-date accumulation removed (requirement: eliminate MTD stats)
...
//             const worked = parseFloat(rec.totalHours) || 0; // stored as totalHours
//             const scheduled = parseFloat(rec.scheduledHours) || 0; // new field (may be undefined for older records)
//             // Month-to-date logic removed
//             // Weekly statistics
...
//                 if (!isNaN(regH)) wRegHours += regH; else wRegHours += worked; // fallback
//                 if (!isNaN(otH)) wOtHours += otH; // if missing treat as 0
//                 // Earnings breakdown (reconstruct if fields exist)
...
//                     // Use computed values to avoid rounding mismatch; guard if sums diverge significantly
...
//         // If breakdown not present (old records), infer regular vs overtime from totals
//         if (wRegHours === 0 && wOtHours === 0 && wWorked > 0) {
//             wRegHours = wWorked;
//         }
//         if ((wRegEarn + wOtEarn === 0) && wEarnings > 0) {
//             // Fallback: all earnings are regular
...
//         // Load user information if not provided in state
//         if (userId) {
...
//         // (Removed local toJsDate; using computeWorkedHours from utils)
...
//     // Consolidate daily total hours & earnings and store in RecordEarnings collection (with overtime logic)
//     useEffect(() => {
...
//         if (hourlyRate <= 0) return; // nothing to store if wage not set

//         const threshold = parseFloat(overtimeSettings.thresholdHours) || 9999;
//         const overtimePercent = parseFloat(overtimeSettings.overtimePercent) || 0; // percent above base (e.g., 50 => 1.5x)
//         const overtimeMultiplier = 1 + (overtimePercent / 100);

//         const newCache = { ...earningsCache };
//         const writes = [];
//         // Sort dates ascending to compute weekly running totals for overtime split
//         const allDates = Object.keys(groupedSchedules).sort();
//         // Group dates by Monday-based week key (YYYY-Www)
//         const weekMap = {};
//         allDates.forEach(dateKey => {
//             const [y, m, d] = dateKey.split('-').map(Number);
//             const dateObj = new Date(y, m - 1, d);
//             // Monday-based week number calculation
//             const tmp = new Date(dateObj.getTime());
//             const day = (tmp.getDay() + 6) % 7; // 0=Monday ... 6=Sunday
...
//             let cumulativeWeekHours = 0; // regular + overtime combined so far
...
//                 // If neither scheduled nor worked, skip entirely
//                 if (scheduledHours <= 0 && worked <= 0) return;

//                 // Determine regular vs overtime split only for worked > 0
//                 let regularHours = 0;
//                 let overtimeHours = 0;
//                 if (worked > 0) {
...
//                 end = parseDateTime(schedule.endDate, schedule.endHour);
//             } else {
//                 end = parseDateTime(schedule.eventDate, schedule.endHour);
//                 if (end <= start) {
...
//                 color = '#6366f1'; // distinct color for overnight
//             }
//             return {
//                 id: schedule.id,
//                 title: schedule.eventDescription + (schedule.overnight ? ' (🌙)' : ''),
//                 start,
//                 end,
...
//         // Detectar si el check-out fue overnight basado en los tiempos registrados
...
//                 checkInTimestamp: new Date(), // consider serverTimestamp() for consistency
...
//         // Calculate total hours worked (support overnight & timestamps)
//         let checkInDate = new Date(`${reg.eventDate}T${checkInHour}`);
//         let checkOutDate = new Date(`${reg.eventDate}T${timeCheckOut}`);

//         // If marked as overnight or if check-out is less than or equal to check-in, add 24 hours
...
//                 checkOutTimestamp: new Date(), // consider serverTimestamp()
//                 status: 'completed',
//                 eventDescription: shiftDescription || reg.eventDescription
//             });
...
//             // Compare on timeline including potential endDate of each shift
...
//     // Handle clicking on calendar events to show edit/delete options
...
//     // Handle editing a shift from calendar click
...
//                     </div>
//                 </div>
//                 {/* Month-to-date card removed per requirement */}
//             </div>
...
//                                                                                                 // continuation only second segment
...
//                                                                     {/* Show break time between shifts */}
...
//                                 min={new Date(0, 0, 0, 0, 0, 0)}        // comienzo del día: 00:00
//                                 max={new Date(0, 0, 0, 23, 59, 0)}      // fin del día: 23:59
//                                 selectable={true}
...
//                                                     // Si se marca como overnight o si el checkout es menor o igual al checkin
//                                                     if (checkOutOvernightEdit || checkOutDate <= checkInDate) {
//                                                         checkOutDate = new Date(checkOutDate.getTime() + 24 * 60 * 60 * 1000);
//                                                     }
...
//             {/* Removed local overtime modal (managed globally) */}
